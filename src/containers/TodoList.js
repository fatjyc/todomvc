...
import { getTodos, newTodo } from "../reducers/action";
import TodoItem from "./TodoItem";

export default function TodoList() {
  const router = useRouter();

  const [todos, { init, addTodo, deleteTodo, setDone }] = useTodos();

  useEffect(() => {
    getTodos().then((todos) => init(todos));
  }, []);
  ...
  const onAddTodo = useOnEnter(() => {
    if (newValue) {
      newTodo(newValue).then((todo) => addTodo(todo));
      setNewValue("");
    }
  }, [newValue]);
